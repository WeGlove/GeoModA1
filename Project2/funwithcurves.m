...
if myhandles.checked % if the checkbox is checked show the intermediate steps
 ...
 %%Your code for drawing the intermediate decasteljau steps (4.1.b) can be put here   
    ...
    X = myhandles.controlPts(:,1);
    Y = myhandles.controlPts(:,2);
    
    savepoints = myhandles.controlPts;
    myt = myhandles.t;
    ...
    for i =1:npoints
        ...
        for l = 1:(numnewp)
            itpoints(l,1) = convexCombination(savepoints(l,1),savepoints(l+1,1),myt);
            itpoints(l,2) = convexCombination(savepoints(l,2),savepoints(l+1,2),myt);
        end
...
function mycurve= my_nice_function(controlPts,tsampling)
    numsamples = numel(tsampling);
    step = 1/numsamples;
    
    mycurve = zeros(numsamples,2);
    for k = 0:step:1
        p = deCasteljau(controlPts,k);
        mycurve(round(k*numsamples+1),1) = p(1);
        mycurve(round(k*numsamples+1),2) = p(2);
    end
    disp(mycurve);
